...
- **Email sending is synchronous in Phase 1.** The Resend call happens inside the request lifecycle of `/api/register`, with a 5-second timeout and one retry. If both attempts fail, the response is 503; the `PENDING` row remains and is reachable by an idempotent retry (§8.6.1). The retention purge in §14.1 sweeps unconsumed `PENDING` rows older than 24 hours independently of the activation-lifecycle purge. There is no queue. See §10.
...
│       └── root.ts                              # appRouter
├── workers/
│   ├── retentionPurge.ts                        # Daily — Railway cron 03:00 UTC (§14.1.1)
│   └── pendingPurge.ts                          # Hourly — Railway cron, minute 7 (§14.1.2)
├── types/
...
A note on the `workers/` directory: neither script is a long-lived process. Both are one-shot scripts invoked by Railway's cron scheduler (see §14.1 / §11). Each connects to Postgres, runs its purge, exits. Phase 1 has no long-lived worker.
...
#### 8.6.1 What Happens When the Email Send Fails

If `emailProvider.sendOtp` fails after the retry inside `lib/email/resend.ts`, the route returns 503. The `PENDING` row created (or upserted) earlier in the handler **remains in the database**. This is intentional — the alternative (deleting the row in a compensating step) introduces concurrency and recovery problems that are worse than a brief orphaned row.

The mental model is **"insert, then send; on failure, idempotent retry"**, not transactional rollback. The trade-off:

- **No DB transaction wrapping the email send.** A Prisma `$transaction` held across a 5-second timeout + retry would starve the connection pool under load. The send happens after the upsert, with no surrounding transaction.
- **The participant's retry is naturally idempotent.** They submit the same email; the upsert finds the existing row; the OTP issuer (§8.7) overwrites the prior Redis key (its multi unconditionally `del`s the key first); the email send is attempted again. Eventually one of three things happens: the email sends and the participant verifies; the participant gives up (orphaned `PENDING`); or 24 hours pass and the dedicated PENDING sweeper purges the row (§14.1.2).
- **No "best-effort delete" on failure.** If the worker dies between "send failed" and "delete the row," you have an orphaned row anyway with no recovery. A delete that runs on a successful failure detection but races a concurrent retry can also delete a freshly re-created row, which is worse than an orphan.

This means a `PENDING` row in the database does not necessarily mean "the participant got an email." It means "the participant submitted, and at insertion time the system attempted to issue an OTP." For audit purposes, the OTP issuance and email-send outcomes can be reconstructed from structured logs (§13.2); the `Registration` table is not the source of truth for "did the email arrive."
...
**Not applicable in Phase 1 in the long-lived sense.** Email send is synchronous (§10). The two retention workers (§14.1.1 daily, §14.1.2 hourly) are one-shot scripts invoked by Railway's cron scheduler — not long-lived processes.

The Phase 1 cron schedule:
...
# to avoid clashing with the top-of-hour load other systems tend to land on.
7 * * * *    pnpm tsx workers/pendingPurge.ts
```

Both scripts connect to Postgres, run their query, and exit. If either fails, its stderr lands in Railway logs; a manual re-run is the recovery. The two are deliberately separate scripts (§14.1.2 — failures should not couple).
...
|------------|-----------|-----------|
| `Registration` (`PENDING`, never verified) | 24 hours from `registeredAt` | Hourly cron, hard delete |
...
The 24-hour `PENDING` sweep is independent of the activation-lifecycle purge below it. The 24h window is a deliberate trade-off: long enough that a participant who started registration on the train and finishes on home wifi can still complete (most of these are in single digits of minutes), short enough that the database doesn't accumulate a long tail of half-finished attempts. A `PENDING` row older than 24h has effectively been abandoned — the OTP itself expired after 10 minutes (§8.7), so the row no longer corresponds to a recoverable session.

The sweep does not write per-row audit entries (high volume, low signal). It writes a single summary `AuditLog` row per run (`action = "retention.purge.pending"`, `metadata.count`).

### 14.1.1 Daily Retention Purge
...
### 14.1.2 Hourly PENDING Sweep

A separate, lightweight worker that runs every hour and purges abandoned `PENDING` registrations older than 24 hours. Lives in its own file so the cron schedules don't get tangled and so a failure in one doesn't block the other.

```ts
// workers/pendingPurge.ts
import { PrismaClient } from "@prisma/client";
...
async function main() {
  ...
  // Single summary row per run, not per deleted registration. Per-row audit
  // would be high-volume and low-signal — these are abandoned sessions, not
  // user-driven actions.
  if (result.count > 0) {
    await prisma.auditLog.create({
      data: {
...
| Wrap an email send (Resend, or any external HTTP call) inside a Prisma `$transaction` | Holds DB connections across a multi-second external call; under load the connection pool starves | Insert/upsert the row, then send outside any transaction; on failure return 503 and let the participant retry idempotently (§8.6.1) |
| Compensating-delete a `Registration` row when its OTP email fails to send | Races concurrent retries (can delete a freshly recreated row) and offers no recovery if the worker dies between detection and delete | Leave the row; the participant retry is idempotent and the §14.1.2 sweep purges abandoned `PENDING` rows after 24h |
...
- [ ] Resend forced to fail twice → 503 returned; the `PENDING` row remains in the DB (per §8.6.1); a subsequent retry from the same participant succeeds without creating a duplicate row
- [ ] No `process.env.*` access outside `lib/env.ts`, `prisma/seed.ts`, `app/instrumentation.ts`
...
- `workers/retentionPurge.ts` per §14.1.1 (already drafted in Phase 2; review, ship)
- `workers/pendingPurge.ts` per §14.1.2 (new in Phase 8 — the abandoned-PENDING sweep)
- Railway cron schedules configured per §11 (both the daily 03:00 UTC and the hourly minute-7)
...
- [ ] PENDING sweep runs hourly against staging; a forced abandoned `PENDING` row older than 24h gets purged; an `AuditLog` row with `action = "retention.purge.pending"` records the run (only when count > 0)
